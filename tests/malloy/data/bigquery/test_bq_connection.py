...
@pytest.mark.parametrize("field_name,field_type,expected_type,expected_num_type", type_test_data)
def test_maps_db_types(field_name, field_type, expected_type, expected_num_type):
    conn = BigQueryConnection()
    field_metadata = namedtuple('metadata', { "name", "field_type" });
    fields = conn._map_fields([field_metadata(name=field_name, field_type=field_type)]);
    
    print(fields)
    assert fields[0] is not None, ("Database column not found: {}".format(field_name))
